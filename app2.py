...
GROQ_API_KEY = "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"
...
def process_vision_colorful(hrnet, image_pil, plan):
    ...
    overlay_rgb = res_img_rgb.copy()

    combined_target_mask = np.zeros((h, w), dtype=np.uint8)
    ...
    # 🌟 核心修复点：鲁棒性检查 Constraint Layer

    roi_mask = None

    c_layer = plan.get('constraint_layer')

    # 只有当 c_layer 是字典，且包含 'id' 键时才执行，防止 KeyError
    ...
    for item in targets_to_check:
        ...
        # 同样的，检查 target_layers 里是否有 id，防止 crash
        ...
        if pixel_count > 0:
            ...
            overlay_rgb[current_mask > 0] = color_rgb

            combined_target_mask = cv2.bitwise_or(combined_target_mask, current_mask)
    ...
    if found_items:
        ...
        res_img_rgb = cv2.addWeighted(res_img_rgb, 0.6, overlay_rgb, 0.4, 0)

        contours, _ = cv2.findContours(combined_target_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
